<template>
    <div class="dashboard-container">
            ...
            <div class="dashboard-right-column">
                ...
                <div class="dashboard-card user-management-card">
                    ...
                    <div v-else class="table-responsive">
                        <table class="users-table compact">
                            ...
                            <tbody>
                                <tr v-for="user in displayedUsers" :key="user.id" class="table-row">
                                    ...
                                    <td class="table-cell">
                                        <span class="role-badge" :class="roleBadgeClasses(user.role)">
                                            {{ getRoleLabel(user.role) }}
                                        </span>
                                    </td>
                                    <td class="table-cell actions-cell">
                                        <div class="table-actions">
                                            <button @click="viewWorkStats(user)" class="table-action-button stats"
                                                title="Statistiques de travail" :disabled="isLoadingWorkStats(user.id)">
                                                <ChartBarIcon />
                                            </button>
                                            <button @click="editUser(user)" class="table-action-button edit"
                                                title="Modifier">
                                                <PencilSquareIcon />
                                            </button>
                                            <button v-if="authStore.isAdmin && user.id !== authStore.user?.id"
                                                @click="deleteUser(user)" class="table-action-button delete"
                                                title="Supprimer">
                                                <TrashIcon />
                                            </button>
                                        </div>
                        ...
                        <!-- View All Users -->
                        <div v-if="userStore.users.length > 5" class="view-all-container">
                            <router-link to="/admin/users" class="view-all-link">
                                Voir tous les {{ userStore.totalUsers }} utilisateurs →
                            </router-link>
                        </div>
                    </div>
                </div>
        ...
        <!-- Pagination -->
        <div v-if="userStore.totalUsers > 0 && userStore.users.length > 5" class="pagination-container">
        ...
        <!-- Work Stats Modal -->
        <div v-if="showWorkStatsModal" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity" @click="closeWorkStatsModal"></div>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
                    ...
                    <div class="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-600 mt-1">{{ selectedUserForStats?.name }}</p>
                            </div>
                            <button @click="closeWorkStatsModal"
                    ...
                    <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        <!-- Loading State -->
                        <div v-if="workStatsLoading" class="flex flex-col items-center justify-center py-12">
                            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mb-4"></div>
                            <p class="text-gray-600">Chargement des statistiques...</p>
                        </div>

                        <!-- Error State -->
                        <div v-else-if="workStatsError" class="flex flex-col items-center justify-center py-12">
                            <ExclamationTriangleIcon class="w-16 h-16 text-red-500 mb-4" />
                            <h3 class="text-sm font-semibold text-gray-900 mb-2">Erreur de chargement</h3>
                            <p class="text-gray-600 text-center max-w-md">{{ workStatsError }}</p>
                        </div>
                        ...
                        <div v-else-if="workStatsData">
                            ...
                            <div
                                ...
                                <div class="relative">
                                    <div v-if="selectedUserForStats?.avatar_url"
                                        class="w-20 h-20 rounded-full bg-cover bg-center border-4 border-white shadow-lg"
                                        :style="{ backgroundImage: `url(${selectedUserForStats.avatar_url})` }">
                                    </div>
                                    <div v-else
                                ...
                                <div class="text-center sm:text-left">
                                    <h3 class="text-2xl font-bold text-gray-900">{{ selectedUserForStats?.name }}</h3>
                                    <p class="text-gray-600 mt-1">{{ selectedUserForStats?.email }}</p>
                                    <div class="mt-3">
                                        <span :class="[
                                            'px-4 py-1.5 rounded-full text-xs font-semibold',
                                            selectedUserForStats?.role === 'admin' ? 'bg-red-100 text-red-800' :
                                                selectedUserForStats?.role === 'manager' ? 'bg-blue-100 text-blue-800' :
                                                    'bg-gray-100 text-gray-800'
                                        ]">
                                            {{ getRoleLabel(selectedUserForStats?.role) }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            <h4 class="font-bold text-gray-900">Aujourd'hui</h4>
                                            <p class="text-xs text-gray-500">{{ new Date().toLocaleDateString('fr-FR')
                                                }}</p>
                                        </div>
                                    </div>
                                    <div class="space-y-4">
                                        <div>
                                            <div class="flex justify-between items-baseline mb-1">
                                                ...
                                                <span class="text-2xl font-bold text-gray-900">
                                                    {{
                                                        formatSecondsToHours(workStatsData.work_time_stats?.today?.total_seconds
                                                            || 0) }}
                                                </span>
                                            </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{
                                                        formatSecondsToHours(workStatsData.work_time_stats?.today?.net_seconds
                                                            || 0) }}
                                                </p>
                                            ...
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{ workStatsData.work_time_stats?.today?.status || 'N/A' }}
                                                </p>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            <h4 class="font-bold text-gray-900">Hebdomadaire</h4>
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <div class="flex justify-between items-baseline mb-1">
                                                ...
                                                <span class="text-2xl font-bold text-gray-900">
                                                    {{
                                                        formatSecondsToHours(workStatsData.work_time_stats?.weekly?.total_seconds
                                                            || 0) }}
                                                </span>
                                            </div>
                                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                                <div class="h-full bg-emerald-500 rounded-full"
                                                    :style="{ width: `${Math.min((workStatsData.work_time_stats?.weekly?.days_worked || 0) * 20, 100)}%` }">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{ workStatsData.work_time_stats?.weekly?.days_worked || 0 }}/5
                                                </p>
                                            ...
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{
                                                        formatHours(workStatsData.work_time_stats?.weekly?.average_daily_hours
                                                            || 0) }}
                                                </p>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-4 mb-6">
                                        ...
                                        <div>
                                            <h4 class="font-bold text-gray-900">Mensuel</h4>
                                            <p class="text-xs text-gray-500">{{ new Date().toLocaleDateString('fr-FR', {
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            <div class="flex justify-between items-baseline mb-1">
                                                ...
                                                <span class="text-2xl font-bold text-gray-900">
                                                    {{
                                                        formatSecondsToHours(workStatsData.work_time_stats?.monthly?.total_seconds
                                                            || 0) }}
                                                </span>
                                            </div>
                                            <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                                                <div class="h-full bg-amber-500 rounded-full"
                                                    :style="{ width: `${Math.min((workStatsData.work_time_stats?.monthly?.days_worked || 0) * 4, 100)}%` }">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{ workStatsData.work_time_stats?.monthly?.days_worked || 0 }}/22
                                                </p>
                                                <p class="text-xs text-gray-500">Jours</p>
                                            </div>
                                            <div class="text-center">
                                                <p class="text-xs font-semibold text-gray-900">
                                                    {{
                                                        formatHours(workStatsData.work_time_stats?.monthly?.average_daily_hours
                                                            || 0) }}
                                                </p>
                                                <p class="text-xs text-gray-500">Moyenne/jour</p>
                                            </div>
                            ...
                            <!-- Recent Sessions -->
                            <div v-if="workStatsData.recent_sessions?.length > 0"
                                ...
                                <div class="divide-y divide-gray-100">
                                    <div v-for="(session, index) in workStatsData.recent_sessions" :key="session.id"
                                        class="p-4 hover:bg-gray-50 transition-colors">
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                            <div class="flex items-center gap-3">
                                                <div :class="[
                                                    'flex-shrink-0 w-3 h-3 rounded-full',
                                                    session.type === 'work' ? 'bg-indigo-500' : 'bg-amber-500'
                                                ]"></div>
                                                <div>
                                                    <span :class="[
                                                        'px-3 py-1 rounded-full text-xs font-semibold',
                                                        session.type === 'work' ? 'bg-indigo-100 text-indigo-800' : 'bg-amber-100 text-amber-800'
                                                    ]">
                                                    ...
                                                    <p class="text-xs text-gray-500">
                                                        Durée: {{
                                                            formatSecondsToHours(Math.abs(session.duration_seconds)) }}
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-xs font-medium text-gray-900">{{ session.work_date }}</p>
                                                <p class="text-xs text-gray-500">
                                                    Session {{ index + 1 }}
                                                </p>
                                            </div>
                                ...
                                <div v-if="workStatsData.recent_sessions.length > 3"
                                    class="px-6 py-3 border-t border-gray-200 bg-gray-50">
                                    <p class="text-xs text-gray-600 text-center">
                                        Affichage des {{ Math.min(workStatsData.recent_sessions.length, 5) }} sessions
                                        les plus récentes
                                    </p>
                                </div>
                            </div>
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div class="text-center">
                                            <p class="text-xl font-bold">
                                                {{ workStatsData.work_time_stats?.weekly?.days_worked || 0 }}
                                            </p>
                                            <p class="text-xs text-gray-300">Jours/semaine</p>
                                        </div>
                                        <div class="text-center">
                                            <p class="text-xl font-bold">
                                                {{
                                            ...
                                            <p class="text-xs text-gray-300">Heures/semaine</p>
                                        </div>
                                        <div class="text-center">
                                            <p class="text-xl font-bold">
                                                {{
                                            ...
                                            <p class="text-xs text-gray-300">Moyenne/jour</p>
                                        </div>
                                        <div class="text-center">
                                            <p class="text-xl font-bold">
                                                {{ workStatsData.work_time_stats?.today?.status || 'N/A' }}
                                            </p>
                                            <p class="text-xs text-gray-300">Statut actuel</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="sticky bottom-0 bg-white border-t border-gray-200 px-4 py-2">
                        <div class="flex justify-between items-center">
                            <div class="text-xs text-gray-600">
                                Données en temps réel • Dernière mise à jour: il y a quelques secondes
                            </div>
                            <div class="flex gap-3">
                                ...
                                <button v-if="authStore.isAdmin" @click="downloadStats"
                                    class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
                        ...
                        <div v-else class="form-grid">
                            <div class="form-group">
                                <label for="name" class="form-label">
                                    <UserIcon class="inline-block w-4 h-4 mr-1" />
                            ...
                            <div class="form-group">
                                <label for="email" class="form-label">
                                    <EnvelopeIcon class="inline-block w-4 h-4 mr-1" />
                            ...
                            <div class="form-group">
                                <label for="role" class="form-label">
                                    <ShieldCheckIcon class="inline-block w-4 h-4 mr-1" />
                            ...
                            <div class="form-group" v-if="!isEditMode || showPasswordField">
                                <label for="password" class="form-label">
                                    <LockClosedIcon class="inline-block w-4 h-4 mr-1" />
                            ...
                            <div class="form-group" v-if="(!isEditMode || showPasswordField) && formData.password">
                                <label for="password_confirmation" class="form-label">
                                    <KeyIcon class="inline-block w-4 h-4 mr-1" />
                            ...
                            <div class="form-group">
                                <label for="avatar" class="form-label">
                                    <PhotoIcon class="inline-block w-4 h-4 mr-1" />
                                    URL de l'avatar
                                </label>
                    ...
                    <div class="row flex">
                        <div class="flex flex-wrap justify-center gap-4">
                            <button @click="closeModal" :disabled="modalLoading"
                                class="inline-flex items-center border font-sans font-medium text-center transition-all duration-300 ease-in disabled:opacity-50 disabled:shadow-none disabled:cursor-not-allowed data-[shape=pill]:rounded-full data-[width=full]:w-full focus:shadow-none text-xs rounded-md py-2 px-4 shadow-sm hover:shadow-md bg-red-800 border-red-800 text-slate-50 hover:bg-red-700 hover:border-red-700">
                                <XMarkIcon class="mr-2 h-4 w-4 stroke-2" />
                                Annuler
                            </button>

                            <button @click="submitForm" :disabled="modalLoading"
                                class="inline-flex items-center border font-sans font-medium text-center transition-all duration-300 ease-in disabled:opacity-50 disabled:shadow-none disabled:cursor-not-allowed data-[shape=pill]:rounded-full data-[width=full]:w-full focus:shadow-none text-xs rounded-md py-2 px-4 shadow-sm hover:shadow-md bg-slate-800 border-slate-800 text-slate-50 hover:bg-slate-700 hover:border-slate-700">
                                <span v-if="modalLoading" class="button-loading">
                                    <ArrowPathIcon class="spinner w-4 h-4" />
                                </span>
                                <CheckIcon class="mr-2 h-4 w-4 stroke-2" />
...
const viewWorkStats = async (user: User) => {
    ...
    try {
        ...
        workStatsError.value = error.message || 'Erreur lors du chargement des statistiques'
    } finally {
