<template>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div
                class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-8"
            >
                <div
                ...
                >
                    ...
                    <div class="flex-1">
                        <div class="relative group">
                            ...
                            >
                                <svg
                                    class="w-4 h-4 text-gray-400 group-hover/clear:text-gray-600 transition-colors"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                >
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative group">
                            <select
                                v-model="filtres.is_public"
                                @change="chargerEquipes"
            ...
            <!-- État de chargement -->
            <div
                v-if="enChargement && equipes.length === 0"
                class="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm"
            >
                <div class="flex flex-col items-center justify-center py-12">
                    <div class="relative">
                        <div
                            class="w-16 h-16 border-4 border-primary/20 rounded-full"
                        ></div>
                        <div
                    ...
                    >
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-100">
                            ...
                            >
                                ...
                                <span
                                    >{{
                                        pagination.total - mesEquipes.length
                                    }}
                                    autres équipes</span
                                >
                            </div>
                    ...
                    >
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-100">
                            ...
                            >
                                ...
                                <span>{{
                                    mesEquipes.length > 0
                                        ? "Vous possédez celles-ci"
                                        : "Aucune équipe possédée"
                                }}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="equipes && equipes.length > 0">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">
                                Gérez et collaborez avec vos équipes
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-medium text-gray-500">
                                Affichage {{ pagination.from }}-{{
                                    pagination.to
                                }}
                    ...
                    >
                        <TeamCard
                            ...
                            :team="equipe"
                            :can-edit="
                                Number(equipe.owner_id) === authStore.user?.id
                            "
                            :can-delete="
                                Number(equipe.owner_id) === authStore.user?.id
                            "
                            @view="afficherDetailsEquipe"
                            @edit="gererEditionEquipe"
                            @delete="gererSuppressionEquipe"
                        />
                    </div>
        ...
        <TeamCreateModal
            ...
            @created="gererEquipeCreee"
        />
        <TeamDetailModal
            v-if="equipeSelectionneePourDetails"
            :team="equipeSelectionneePourDetails"
            :can-edit="
                equipeSelectionneePourDetails.owner_id === authStore.user?.id
            "
            @close="equipeSelectionneePourDetails = null"
            @edit="gererEditionDepuisDetails"
        />
        <TeamEditModal
            v-if="equipeSelectionnee"
            :team="equipeSelectionnee"
            @close="equipeSelectionnee = null"
            @updated="gererEquipeMiseAJour"
        />
        <TeamDeleteModal
            v-if="equipeASupprimer"
            :team="equipeASupprimer"
            @close="equipeASupprimer = null"
...
// Définir une interface pour la pagination
...
const pagination = computed<PaginationData>(() => {
    ...
    // Si pas de données du store
    if (!storePagination.value) {
        return defaultData;
    }

    // Prendre les données du store
    const { current_page, last_page, per_page, total } = storePagination.value;

    // Calculer from et to (qui ne sont PAS dans storePagination.value)
...
const equipesActivesCount = computed(() => {
    // return equipes.value.length;
    return equipes.value.filter((equipe) => {
        return (equipe.members_count || 0) > 0;
    }).length;
});
...
let rechercheTimeout: ReturnType<typeof setTimeout>;
