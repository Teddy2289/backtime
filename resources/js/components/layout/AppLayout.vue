<template>
    <div class="min-h-screen bg-gray-50">
        ...
        <nav class="bg-white border-b border-gray-200 shadow-sm">
                ...
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="flex items-center space-x-8">
                        ...
                        <div class="hidden lg:flex items-center space-x-1">
                            <router-link v-for="item in mainNavigation" :key="item.name" :to="item.to" :class="[
                            ...
                            <div class="relative" data-menu>
                                <button @click="toggleNotifications"
                                    ...
                                    <span v-if="unreadNotifications > 0"
                                        class="absolute -top-0.5 -right-0.5 h-2 w-2 bg-red-500 rounded-full"></span>
                                </button>
                                ...
                                <div v-if="showNotifications"
                                    ...
                                    <div class="px-4 py-2 border-t border-gray-100">
                                        <router-link :to="{ name: 'notifications' }" @click="closeAllMenus"
                            ...
                            <div class="relative" data-menu>
                                <button @click="toggleUserMenu"
                                    ...
                                    <div
                                        ...
                                        <span class="text-[10px] font-semibold text-white">{{ authStore.user?.initials
                                            || 'U' }}</span>
                                    </div>
                                ...
                                <div v-if="showUserMenu"
                                    ...
                                    <div class="px-3 py-2 border-b border-gray-100">
                                        <p class="text-xs font-medium text-gray-500 mb-1">Connecté en tant que</p>
                                        <p class="text-sm font-semibold text-gray-900 truncate">{{ authStore.user?.name
                                            || 'Utilisateur' }}</p>
                                        <p class="text-xs text-gray-500 truncate">{{ authStore.user?.email ||
                                            'email@exemple.com' }}</p>
                                    </div>
                                    <div class="py-1">
                                        <router-link :to="{ name: 'profile' }" @click="closeAllMenus"
                                        ...
                                        <router-link :to="{ name: 'admin.settings' }" @click="closeAllMenus"
            ...
            <div v-if="showMobileMenu" class="lg:hidden bg-white border-t border-gray-100 px-4 py-3">
                <div class="space-y-1">
                    <router-link v-for="item in mainNavigation" :key="item.name" :to="item.to" @click="closeAllMenus"
                        :class="[
                        ...
                        <span v-if="item.badge" class="bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded-full">
                            {{ item.badge }}
                        </span>
                    </router-link>
                </div>
            ...
            <div class="container mx-auto px-4">
                ...
                <nav class="mb-6">
                    <ol class="flex items-center space-x-1 text-xs text-gray-500">
                        <li>
                            <router-link :to="{ name: 'dashboard' }" class="hover:text-gray-700">Dashboard</router-link>
                        </li>
                        <li class="flex items-center">
                            <ChevronDownIcon class="h-3 w-3 transform -rotate-90" />
                        </li>
                        <li class="text-gray-900 font-medium">{{ currentPageTitle }}</li>
                    </ol>
                </nav>
        ...
        <div v-if="showUserMenu || showNotifications" class="fixed inset-0 z-40" @click="closeAllMenus"></div>
    </div>
</template>
...
const handleClickOutside = (event: MouseEvent) => {
    ...
    const isMenuElement = target.closest('[data-menu]') ||
